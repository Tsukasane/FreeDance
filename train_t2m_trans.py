...
class TextCLIP(torch.nn.Module): # use clip feature
    def __init__(self, model) :
        super(TextCLIP, self).__init__()
        self.model = model

    ## NOTE(yiwen) word_emb is for token level representation (w/ higher granularity)    
    ## while enctxt is a sentence level representation, 
    ## transformer music encoder/ hubert discrete features
...
if len(os.listdir(codebook_dir)) == 0:
    ...
    for batch in train_loader_token:
        pose, name = batch
        # pose, name, mel= batch 
        # torch.Size([1, 60, 251]) use a shorter sequence here
        # ('00579',)
        # tensor([60])
        ...
        target = target.cpu().numpy() # motion code target 定长 window_length/unit_length
        
        #target.shape (1, 15)
        #pose.shape torch.Size([1, 60, 251])
...
for nb_iter in range(1, args.total_iter + 1):
    ...
    clip_text, m_tokens, m_tokens_len = batch # list len=128, torch.Size([128, 50]), torch.Size([128])
    # m_tokens_len 是不足50的原长， pad到50，m_tokens
    ...
    cls_pred = trans_encoder(masked_input_indices,  # 128, 50
                             feat_clip_text,  # 128, 512
                             src_mask = seq_mask,  # 128, 50
                             att_txt=att_txt, 
                             word_emb=word_emb)[:, 1:]  # 128, 77, 512
