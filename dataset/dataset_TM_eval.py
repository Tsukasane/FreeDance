...
class Text2MotionDataset(data.Dataset):
    def __init__(self, dataset_name, is_test, w_vectorizer, feat_bias = 5, max_text_len = 20, unit_length = 4, shuffle=True):
        ...
        if dataset_name == 't2m':
            ...
            #TODO(yw) collect meta data of aistpp
            pass
            ...
            try:
                ...
                with cs.open(pjoin(self.text_dir, name + '.txt')) as f:
                    for line in f.readlines():
                        ...
                        tokens = line_split[1].split(' ') # [person/NOUN,...]
                        f_tag = float(line_split[2]) #NOTE(yw) start time, end time
                        ...
                        # {'caption': 'A Person doing a tennis racket like movement', 'tokens': ['A/DET', 'person/NOUN', 'do/VERB', 'a/DET', 'tennis/NOUN', 'racket/NOUN', 'like/SCONJ', 'movement/NOUN']}
                        
                        if f_tag == 0.0 and to_tag == 0.0: #NOTE(yw) 一般整段可用
                            flag = True
                            ...
                            try:
                                ...
                                if (len(n_motion)) < min_motion_len or (len(n_motion) >= 200):
                                    continue
                                new_name = random.choice('ABCDEFGHIJKLMNOPQRSTUVW') + '_' + name #NOTE(yw) window采到同一段序列
    ...
    def __getitem__(self, item):
        ...
        if m_length < self.max_motion_length and self.shuffle: # pad zero
