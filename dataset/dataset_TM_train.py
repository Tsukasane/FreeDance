...
class Text2MotionDataset(data.Dataset):
    def __init__(self, dataset_name, feat_bias = 5, unit_length = 4, codebook_size = 1024, tokenizer_name=None, up_low_sep=False):
        ...
        for name in id_list:
            try:
                m_token_list = np.load(pjoin(tokenizer_name, '%s.npy'%name)) # motion tokens
                #NOTE(yw) tokenizer_name is codebook_dir ./output/vq/2024-12-02-03-46-11_vq_name/codebook/
                # Read text
                    ...
                    for line in lines:
                        try:
                            ...
                            if f_tag == 0.0 and to_tag == 0.0:
                                ...
                                # TODO(yw) check whether text tokens or motion tokens NOTE (yw) cut corresponding text token
                                m_token_list_new = [tokens[int(f_tag*fps/unit_length) : int(to_tag*fps/unit_length)] for tokens in m_token_list if int(f_tag*fps/unit_length) < int(to_tag*fps/unit_length)] # NOTE(yw) motion token and text token correspondance
                                # num of tokens
                                ...
                                data_dict[new_name] = {'m_token_list': m_token_list_new, #NOTE(yw) 一整段motion token，一整段music feature
                                                       'text':[text_dict]}
    ...
    def __getitem__(self, item):
        ...
        m_tokens = random.choice(m_token_list) # NOTE(yw) all elements in the list have the same meanings?

        text_data = random.choice(text_list)
        caption= text_data['caption'] #NOTE(yw) 有对应的几段描述，随机选一种
        ...
        if coin:
            ...
            if coin2:
                m_tokens = m_tokens[:-1] # drop tail
            else:
                m_tokens = m_tokens[1:] # drop head
        ...
        return caption, m_tokens, m_tokens_len # text token + motion token + motion token length
