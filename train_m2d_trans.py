...
if args.dataname == 'aamixed': 
    dataset_opt_path = '/home/xingqunqi/AI_dance/AI_dance/checkpoints/aamixed/opt.txt' 

elif args.dataname == 'aistpp':
    dataset_opt_path = '/home/xingqunqi/AI_dance/AI_dance/checkpoints/aistpp/opt.txt' 

elif args.dataname == 'aioz':
    dataset_opt_path = '/home/xingqunqi/AI_dance/AI_dance/checkpoints/aioz/opt.txt'
...
if len(os.listdir(codebook_dir)) == 0:
    ...
    for batch in train_loader_token:
        pose, _, name, _, num_person = batch                                                    #----------TODO： litingw----这里得到num_person（后面可能用上，dev用上了）
        ...
        # TODO(yiwen) codebook 在加人数之后需要specific design                                    #----------TODO： litingw（改完后删掉这行）
...
for nb_iter in tqdm(range(iter_start, args.total_iter + 1), position=0, leave=True):                    # ---------NOTE： litingw 训练 transformer
