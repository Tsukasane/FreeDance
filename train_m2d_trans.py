...
for epoch in range(epoch_start, args.num_epochs):
    ...
    scheduler.step()  
    logger.info(f"Epoch {epoch+1}/{args.num_epochs}, Learning rate: {optimizer.param_groups[0]['lr']}")

    if epoch % args.print_epoch ==  0 or epoch==args.num_epochs:
        msg_epocheval= f"Eval on {epoch} epoch!"
        logger.info(msg_epocheval)
        num_repeat = 1
        rand_pos = False
        if nb_iter == args.total_iter:
            num_repeat = -30
