...
class QuantizeEMAReset2D(nn.Module):
    def __init__(self, nb_code, max_person, code_dim, args=None):
        super().__init__()
        self.max_person = max_person # new_dim
        self.nb_code = nb_code # index
        self.code_dim = code_dim
        self.mu = args.mu
        self.reset_codebook()
        self.reset_count = 0
        self.usage = torch.zeros((self.nb_code, 1)) # 2D code will be used as a whole: nb_code_x * nb_code_y
    ...
    def reset_codebook(self):
        ...
        self.code_count = None
        # zero initialize
        self.register_buffer('codebook', torch.zeros(self.nb_code, self.max_person, self.code_dim).cuda())

    def _tile(self, x):
        nb_code_x, h, width = x.shape 
        if nb_code_x < self.nb_code:
            n_repeats = (self.nb_code + nb_code_x - 1) // nb_code_x
            std = 0.01 / np.sqrt(h * width)  
            out = x.repeat(n_repeats, 1, 1)[:self.nb_code]
            out = out + torch.randn_like(out) * std
        else:
            out = x[:self.nb_code]
        return out
    ...
    def init_codebook(self, x):
        """
        input: (NT, H, width)
        output: codebook (nb_code, H, width)
        """
        out = self._tile(x)

        # init codebook, code_sum (same shape)
        self.codebook = out.clone()  # (nb_code, H, width)
    ...
    def dequantize(self, code_idx):
        x = self.codebook[code_idx] # --> NT, H, width
        return x
    ...
    def update_codebook(self, x, code_idx):
        ...
        code_update = code_update.view(self.nb_code, x.shape[1], x.shape[2]) # 8192, 3, 32

        out = self._tile(x)
        # --> 8192, 3, 32
        code_rand = out[torch.randperm(out.shape[0])[:self.nb_code]].view(self.nb_code, x.shape[1], x.shape[2])

        # Update centres
        code_sum = code_sum.view(code_sum.shape[0], self.code_sum.shape[1], self.code_sum.shape[2])
        self.code_sum = self.mu * self.code_sum + (1. - self.mu) * code_sum  # nb_code, H, width
        self.code_count = self.mu * self.code_count + (1. - self.mu) * code_count  # nb_code
        ...
        if self.reset_count >= 20:
            ...
            self.reset_count += 1
            self.usage = (usage + self.usage >= 1.0).float()
            usage = torch.ones_like(self.usage, device=x.device) # nb_code, 1
        code_update = self.code_sum.view(self.nb_code, -1) / self.code_count.view(self.nb_code, 1) # nb_code, H*width
        code_update = code_update.view(code_update.shape[0], code_rand.shape[1], code_rand.shape[2])
        usage = usage.unsqueeze(2)
        self.codebook = usage * code_update + (1 - usage) * code_rand
        prob = code_count / torch.sum(code_count)  
        perplexity = torch.exp(-torch.sum(prob * torch.log(prob + 1e-7)))
    ...
    def forward(self, x):
        ...
        # Preprocess
        x = self.preprocess(x) # NT, H, width
