...
class VQVAE_SEP(nn.Module): #NOTE(yw) upper body and lower body use separate codebooks
    def __init__(self,
                 args,
                 nb_code=512,
                 code_dim=512,
        ...
        if args.dataname == 'kit':
            ...
            self.nb_joints = 24
            output_dim = '' #TODO(yw) how to decide the output dim
            upper_dim = ''
            lower_dim = ''
    ...
    def rand_emb_idx(self, x_quantized, quantizer, idx_noise):
        ...
        x_quantized = x_quantized.permute(0,2,1) #NOTE(yw) mask(baseline 不用mask) 输入维度，（B,1,150,72）决定encoder结构
    ...
    def shift_upper_up(self, data): #NOTE(yw)upper+lower一起
        _data = data.clone()
        data = self.denormalize(data)
    ...
    def forward(self, x, *args, type='full', **kwargs):
        ...
        if type=='full':
            ...
            #upper_emb = x[..., SMPL_UPPER_BODY_MASK] #NOTE(yw) if using aistpp
            #lower_emb = x[..., SMPL_LOWER_BODY_MASK] 
            ...
            lower_emb = x[..., HML_LOWER_BODY_MASK]
            #lower_emb = x[..., SMPL_LOWER_BODY_MASK] #NOTE(yw) if using aistpp
            lower_emb = self.preprocess(lower_emb)
            lower_emb = self.encoder_lower(lower_emb)
...
def merge_upper_lower(upper_emb, lower_emb):
    ...
    # motion[..., SMPL_UPPER_BODY_MASK] = upper_emb
    # motion[..., SMPL_LOWER_BODY_MASK] = lower_emb
    return motion
...
def upper_lower_sep(motion, joints_num):
    ...
    positions = positions.view(*motion.shape[:2], (joints_num - 1), 3) #NOTE(yw)shape不同
