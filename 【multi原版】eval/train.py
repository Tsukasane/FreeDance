...
def train_epochs(args, train_loader, dataset_name, device):
    ...
    for epoch in range(args.epochs):
        ...
        for iter_idx, data in enumerate(train_loader, 0):
            gt_motion, _, _, _ = data 
            gt_motion = gt_motion.cuda().float() 
            pose_seq = gt_motion  
            B, H, T, D = pose_seq.shape
            pose_seq = pose_seq.view(B, H*T, D) #TODO(yw) check implementation as vqvae training
            ...
            if train_steps % 100 == 0:
                ...
                visualize_joints(positions[0,0,:,:], save_name='./eval/ae_gt.png') # (24, 3)
                ...
                visualize_joints(positions_recons[0,0,:,:], save_name='./eval/ae_recons.png') # (24, 3)
...
def main(args):
    ...
    train_dataset = Music2DanceDataset_AE(dataset_name, is_test=False, shuffle=True, normalizer=None) #TODO(yw) check why trainset doesn't need normalizer
    normalizer=train_dataset.normalizer
    val_dataset = Music2DanceDataset_AE(dataset_name, is_test=True, shuffle=True, normalizer=normalizer)# use the testset, since aistpp has no val set
...
if __name__=='__main__':
    ...
    parser.add_argument("--ori_joints", default="beat_smplx_joints", type=str)
    parser.add_argument("--tar_joints", default="beat_smplx_full", type=str)
    parser.add_argument("--pose_length", default=150, type=int)    
    
    # ------------------ dataset constraction------------
    parser.add_argument("--pose_fps", default=15, type=int)
    parser.add_argument("--pose_rep", default="6D_smplx", type=str)
    parser.add_argument("--dataset_name", default='aistpp', type=str)
    ...
    parser.add_argument("--checkpoint_dir", type=str, default='/home/xingqunqi/AI_dance/MMM/eval/checkpoints')
    parser.add_argument("--log_save_path", type=str, default='/home/xingqunqi/AI_dance/MMM/eval/logs')
